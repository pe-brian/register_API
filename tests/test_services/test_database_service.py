...
class TestDatabaseService:
    ...
    def setup_class(self):
        with patch('src.services.database_service.getenv') as mock_getenv, \
             patch('src.services.database_service.mysql.connector.connect') as mock_connect, \
             patch.object(DatabaseService, 'dispatch_service', MagicMock()) as mock_dispatch:
            mock_getenv.side_effect = ['test_db', 'test_user', 'test_password', 'test_url']
    ...
    def test_create_table(self, mock_query_create_table):
        mock_query_create_table.return_value.build.return_value = 'CREATE TABLE fake_model'
    ...
    def test_create_or_update_object(self):
            with patch('src.services.database_service.DatabaseService.update_object') as mock_update_object,\
                patch('src.services.database_service.DatabaseService.create_object') as mock_create_object:
    ...
    def test_create_object(self, mock_insert_row, mock_asdict):
        ...
        mock_insert_row.return_value.build.return_value = 'INSERT INTO test_table (field) VALUES (value)'
        self.db_service._models = {'fake_model': FakeModel}
        self.db_service.create_object('fake_model', test_field="test")
        self.mock_db_cursor.execute.assert_called_with('INSERT INTO test_table (field) VALUES (value)')

    @patch('src.services.database_service.Query.delete_rows')
    def test_delete_objects(self, mock_delete_rows):
        mock_delete_rows.return_value.build.return_value = 'DELETE FROM test_table WHERE id = 1'
        condition = MagicMock()
        self.db_service.delete_objects('test_table', condition)
        mock_delete_rows.assert_called_with(table='test_table', filter=condition)
        self.mock_db_cursor.execute.assert_called_with('DELETE FROM test_table WHERE id = 1')
    ...
    def test_connect_to_db(self, mock_connect):
        ...
        mock_connect.assert_called_with(user='user', password='password', host='host', database='db', charset='utf8', autocommit=True)
    ...
    def test_filter(self, mock_get_rows):
        mock_get_rows.return_value.build.return_value = 'SELECT * FROM test_table WHERE id = 1'
        condition = MagicMock()
        ...
        self.mock_db_cursor.execute.assert_called_with('SELECT * FROM test_table WHERE id = 1')
        assert result.check()

    def test_exec_query(self):
        with patch('src.services.database_service.DatabaseService.dispatch_service') as mock_dispatch_service:
            ...
            mock_dispatch_service.dispatch.assert_called_with("QUERY_EXECUTED", query=query)
