...
class TestCodeSenderService:
    ...
    def test_send_activation_code(self, mock_user_get):
        with patch.object(CodeSenderService, 'dispatch_service', MagicMock()) as mock_dispatch:
            with patch.object(CodeSenderService, 'smtp_service', MagicMock()) as mock_smtp:
                ...
                mock_smtp.send_email.assert_called_once_with("Activation code", '123456', to_emails=[mock_user.email])
                mock_dispatch.dispatch.assert_called_once_with("ACTIVATION_CODE_SENT", code=mock_activation_code.code, user_id=mock_user.id)
