...
class TestLoggingService:
    ...
    def test_log_info(self, mock_logger: tuple[Logger, StringIO]):
        ...
        with patch('src.services.logging_service.logging.getLogger', return_value=logger):
    ...
    def test_on_event_info(self, mock_logger: tuple[Logger, StringIO]):
        ...
        with patch('src.services.logging_service.logging.getLogger', return_value=logger):
            ...
            stream.seek(0)
            assert "[INFO] FAKE_EVENT (args: ('1',) | kwargs: {'param': '2'}))" in stream.getvalue()
    ...
    def test_on_event_error(self, mock_logger: tuple[Logger, StringIO]):
        ...
        with patch('src.services.logging_service.logging.getLogger', return_value=logger):
            ...
            stream.seek(0)
            assert "[ERROR] ERROR_FAKE_EVENT (args: ('1',) | kwargs: {'param': '2'}))" in stream.getvalue()
