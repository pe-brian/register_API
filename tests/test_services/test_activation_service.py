...
from src.services.activation_service import ActivationService, CodeExpiredError, WrongCodeError
...
class TestActivationService:
    ...
    def test_activate_correct_code(self, mock_get, user_mock: MagicMock, activation_code_mock: MagicMock):
        mock_get.return_value = activation_code_mock
        with patch.object(ActivationService, 'dispatch_service', MagicMock()) as mock_dispatch:
            service = ActivationService()
            service.activate(user_mock, '1234')
            mock_dispatch.dispatch.assert_called_once_with("USER_ACCOUNT_ACTIVATED", id=user_mock.id)
            assert user_mock.is_active

    @patch('src.services.activation_service.ActivationCode.get')
    def test_activate_incorrect_code(self, mock_get, user_mock: MagicMock, activation_code_mock: MagicMock):
        mock_get.return_value = activation_code_mock
        service = ActivationService()
    ...
    @patch('src.services.activation_service.ActivationCode.get')
    def test_activate_expired_code(self, mock_get, user_mock: MagicMock, expired_activation_code_mock: MagicMock):
