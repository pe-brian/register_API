...
class DatabaseService(Service):
    ...
    def __init__(self) -> None:
        ...
        if not db or not user or not password or not url:
            raise EnvironmentError("MYSQL_DB, MYSQL_USER, MYSQL_PASSWORD & MYSQL_URL environement variables must be defined")
        self._db_name = db
        self._models = {}
    ...
    def get_model_cols(self, model) -> Tuple[Col]:
        ...
        return tuple(Col(name=field.name, type=cast_to_sql_type(field.type)) for field in fields(model) if field.name != "id")
    
    def create_table(self, table_name: str) -> None:
        """ Create the table according to the registered models """
        model = self._models[table_name]
        req = Query.create_table(table=table_name, cols=self.get_model_cols(model), auto_id=True).build()
        self.exec_query(req)
    ...
    def update_object(self, table_name: str, *args, **kwargs) -> BaseModel:
        ...
        self.exec_query(Query.update_rows(table_name, *args, filters=[Col("id") == kwargs["id"]], assignations=assignations).build().replace("(", "").replace(")", ""))
    ...
    def create_object(self, table_name: str, *args, **kwargs) -> BaseModel:
        ...
        query = Query.insert_row(table=model.get_table_name(), cols=self.get_model_cols(model), row=row).build()
    ...
    def delete_object_by_id(self, table_name: str, id: int) -> None:
        ...
        self.exec_query(Query.delete_rows(table=table_name, filter=(Col("id") == id).build()))

    def connect_to_db(self, user: str, password: str, host: str, db: str):
        """ Connect to the MySQL database """
        try:
            db_connection = mysql.connector.connect(user=user, password=password, host=host, database=db, charset="utf8", autocommit=True)
        except mysql.connector.Error as err:
            if err.errno == errorcode.ER_ACCESS_DENIED_ERROR:
                self.dispatch_service.dispatch("ERROR_OCCURRED", emitter="DatabaseService", err="DB_ACCESS_DENIED_ERROR")
            elif err.errno == errorcode.ER_BAD_DB_ERROR:
                self.dispatch_service.dispatch("ERROR_OCCURRED", emitter="DatabaseService", err="DB_NOT_FOUND")
            else:
                self.dispatch_service.dispatch("ERROR_OCCURRED", emitter="DatabaseService", err="DB_UNKNOW_ERROR")
            raise err
        else:
            self.dispatch_service.dispatch("DB_CONNECTION_ESTABLISHED", emitter="DatabaseService")
